...
def f(x):#just put in two digit numbers with a space between them
    ...
    for row in range(0,17):#diagonals left to right
        for col in range(0,17):
            products.append(matrix[row][col]*matrix[row+1][col+1]*matrix[row+2][col+2]*matrix[row+3][col+3])
    for row in range(0,17):#diagonals left to right
        for col in range(3,20):
            products.append(matrix[row][col]*matrix[row+1][col-1]*matrix[row+2][col-2]*matrix[row+3][col-3])
    for row in range(0,17):#vertical
        for col in range(0,20):
            products.append(matrix[row][col]*matrix[row+1][col]*matrix[row+2][col]*matrix[row+3][col])
    for row in range(0,20):#diagonals left to right
        for col in range(0,17):
            products.append(matrix[row][col]*matrix[row][col+1]*matrix[row][col+2]*matrix[row][col+3])
